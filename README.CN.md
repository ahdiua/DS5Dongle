...
- 单击 BOOTSEL，可在原生 DualSense 与 Xbox 360 兼容 XInput 模式之间实时切换。
...
- 单击：在原生 DualSense 模式和 Xbox 360 兼容 XInput 模式之间切换
- 双击：扫描/切换已配对的手柄
- 三击：正常重启 Pico
- 四击：重启并进入 BOOTSEL 刷机模式
- 长按约 1.5 秒：清除所有已保存的手柄配对

单击切换后，Pico 会断开并重新枚举 USB 设备，但不会断开当前蓝牙手柄。
...
自适应扳机和网页配置。XInput 输入端点会沿用已保存的回报率设置（250 Hz、
500 Hz 或实时）；由于 XInput 模式下无法使用网页配置，请先在原生 DualSense
模式中设置回报率。此切换不会写入闪存，断电或重启后会自动恢复原生
DualSense 模式；再次单击即可立即切回。XInput 模式仅面向 Windows PC，不能让
适配器连接 Xbox 主机。

启用“按 PS 键唤醒”后，XInput 模式也会声明 USB 远程唤醒能力，并使用下一份
手柄按键报告唤醒主机，但不会枚举原生模式使用的辅助键盘接口。
...
PC”。启用后，原生 DualSense 模式会增加 HID 键盘接口并声明 USB 远程唤醒能力；
XInput 模式则直接通过游戏手柄接口远程唤醒，不会枚举辅助键盘接口。
